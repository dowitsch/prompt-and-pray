...
function edgeMismatch(data: Uint8ClampedArray, w: number, h: number): number {
	...
	for (let y = 0; y < h; y++) {
		...
		const r = (y * w + w - 1) * 4;
		sum +=
			Math.abs(data[l] - data[r]) +
			Math.abs(data[l + 1] - data[r + 1]) +
			Math.abs(data[l + 2] - data[r + 2]);
	}
	for (let x = 0; x < w; x++) {
		const t = x * 4;
		const b = ((h - 1) * w + x) * 4;
		sum +=
			Math.abs(data[t] - data[b]) +
			Math.abs(data[t + 1] - data[b + 1]) +
			Math.abs(data[t + 2] - data[b + 2]);
	}
	return sum / ((w + h) * 3);
}
 ...
 */
/**
 * Box-filter factor applied on load.
 *
 * The spike shipped 1254px files and halved them; `scripts/shrink-textures.mjs`
 * now delivers 640px files that are already at the resolution actually sampled,
 * so halving again would only throw detail away.
 */
export const SURFACE_REDUCE = 1;
...
/**
 * A 2D canvas on whichever thread we are on.
 *
 * `OffscreenCanvas` is preferred outright rather than only as a worker fallback:
 * on the main thread it avoids attaching a throwaway element to nothing, and it
 * is the only option inside the worker, so one path serves both.
 */
function paintable(
	width: number,
	height: number
	...
	if (typeof OffscreenCanvas !== 'undefined') {
		// The settings object is written out at each call site rather than shared:
		// hoisting it into a variable loses the '2d' overload and TS widens the
		// result to the whole union of rendering contexts.
...
async function load(url: string): Promise<Surface | null> {
	try {
		...
		// Surfaces are loaded inside the terrain worker, which has no `document`.
		const ctx = paintable(bitmap.width, bitmap.height);
